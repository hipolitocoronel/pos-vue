<template>
  ...
  <main v-auto-animate>
    ...
    <section v-else class="flex gap-10">
      <section class="grid mt-8 gap-y-10 gap-x-8 grow max-w-7xl lg:grid-cols-2">
        <div class="flex col-span-2 gap-4">
          ...
          <div class="grid w-[50%] items-center gap-1.5 relative">
            <Label for="code" class="mb-1">Código</Label>
            <Input
              id="code"
              type="text"
              placeholder="Ej: CODE1234"
              v-model="store.product.code"
              :disabled="page.showOnly"
            />
            <p class="absolute text-xs text-muted-foreground -bottom-5">
              Código único para identificar tu producto
            </p>
          </div>
        </div>
        ...
        <div class="flex gap-4">
          <div class="grid w-full items-center gap-1.5" v-auto-animate>
            ...
            <Input
              ...
              v-model.number="store.product.stock"
              :disabled="page.showOnly"
            />
            <Label class="text-xs text-red-500" v-if="store.errors['stock']">
              {{ store.errors["stock"] }}
            </Label>
          </div>
          <div class="grid w-full items-center gap-1.5">
            ...
            <Input
              ...
              :disabled="page.showOnly"
            />
            <Label class="text-xs text-red-500" v-if="store.errors['minStock']">
              {{ store.errors["minStock"] }}
            </Label>
          </div>
        </div>
